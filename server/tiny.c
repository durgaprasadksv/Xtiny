...
void snapy_serve_dynamic(xtiny_ctx *server_ctx, int connfd, char* filename, char* cgiargs) {
    ...
    */  
    snapy_thread_args* targs = &server_ctx->args_queue[server_ctx->head];
    targs->connfd = connfd;
    set_snappy_env(server_ctx, cgiargs, targs);
    server_ctx->head = ( server_ctx->head + 1 ) % server_ctx->queue_size;
...
void* snapy_thread_serve_dynamic(void* args) {
    ...
    char sockpath[MAX_DOMAINSOCK_LEN];

    /* Open connection to the dynamic content process */    
    struct sockaddr_un client_sock;
    int fd;
    int socknum =(int) ((long) pthread_self() % domainsock_count);
    if (socknum < 0) socknum = -socknum;
    sprintf(sockpath,"/tmp/%d.sock", socknum);
    ...
    if (connect(fd, (struct sockaddr *)&client_sock, sizeof(struct sockaddr_un)) < 0) {
        close(fd);
        printf("stream socked %s \n ", sockpath);
        perror("connecting stream socket");
        exit(1);
    }
    send_fd(fd, targs->connfd);
    Write(fd, targs, sizeof(snapy_thread_args));
